# M3, kept: the scalar wave equation on a **cell-centered** field set.
#
# This is the M3 study verbatim, with `centering = cellcentered(D)` said
# out loud at every entry point. From M8 on the wave study proper is the
# vertex-centered one in `wave_tests.jl`; this file stays so that the M3
# numbers — the rate table under "Operators" in CODE.md, and the pulse
# tracking of M4 — remain under test rather than becoming history.
#
# The acceptance criterion is unchanged: volume-weighted L2/L∞ errors
# against the exact sine-mode solution converge at 2nd order, which
# needs order-4 operators and G = 2.

@testset "Cell-centered wave equation on a uniform grid: D=$D" for D in (1, 2)
    ...
    for N in (8, 16, 32)
        r = wave_errors(Val(D); N=N, G=1, refined=false, centering=cellcentered(D))
        push!(hs, r.h)
        push!(l2, r.l2)
    end
...
@testset "Cell-centered wave equation on a two-level mesh: D=$D" for D in (1, 2)
...
@testset "Interface order limits the cell-centered rate: D=$D" for D in (1,)
    ...
    # the interface first order. (Along a stagger only prolongation
    # matters, because restriction is injection; see wave_tests.jl.)
    rate(ops, G) = begin
        hs, l2 = Float64[], Float64[]
        for N in (8, 16, 32)
            r = wave_errors(Val(D); N=N, G=G, ops=ops, centering=cellcentered(D))
            push!(hs, r.h)
            push!(l2, r.l2)
        end
...
@testset "Cell-centered wave equation in 3D" begin
...
@testset "A moving cell-centered refined region tracks a propagating pulse" begin
    ...
    σ = 0.08
    C = cellcentered(1)
    coarse = uniform_pulse(Val(1); roots=8, N=8, σ=σ, centering=C)   # level-0 equivalent
    fine = uniform_pulse(Val(1); roots=8, N=32, σ=σ, centering=C)    # level-2 equivalent
...
@testset "A cell-centered RHS does not mutate the state vector" begin
    ...
    fs = FieldSet(forest, 2; G=2, centering=cellcentered(1))
...
@testset "Cell-centered energy stays bounded" begin
